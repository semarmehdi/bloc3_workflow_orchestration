...
def _send_email(subject, body, attachment_path=None, attachment_name=None):
    ...
    app_password = Variable.get("SMTP_APP_PASSWORD")  # cle d'application Google (16 car.)
...
def send_fraud_notification(notify_on_no_fraud=True, **context):
    ...
    n_fraud = int(
        ti.xcom_pull(task_ids="predict_with_model_fraud", key="fraud_detected_count") or 0
    )
    n_total = int(
        ti.xcom_pull(task_ids="predict_with_model_fraud", key="fraud_predictions_count") or 0
    )
    s3_key = ti.xcom_pull(
